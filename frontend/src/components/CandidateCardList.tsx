...
export default function CandidateCardList({candidates, currentPage, setCurrentPage, maxPage, setSelectedCandidate, setVisibleModifySkills, setVisibleRemoveCandidate} : {
    candidates: Candidate[],
    currentPage: number,
    setCurrentPage: React.Dispatch<React.SetStateAction<number>>,
    maxPage: number,
    setSelectedCandidate: React.Dispatch<React.SetStateAction<Candidate | null>>,
    setVisibleModifySkills: React.Dispatch<React.SetStateAction<boolean>>,
    setVisibleRemoveCandidate: React.Dispatch<React.SetStateAction<boolean>>,
}){
    return (
        <div className="mt-[30px] max-w-[80%] mx-auto flex flex-col items-center justify-center gap-[50px] pb-[30px]">
            <div className="w-full grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-[20px]">
                {candidates.map((c, idx) => <CandidateCard onModify={() => {
                    setSelectedCandidate(c)
                    setVisibleModifySkills(true)
                }} onRemove={() => {
                    setSelectedCandidate(c)
                    setVisibleRemoveCandidate(true)
                }}  id={c.id} contactNumber={c.contactNumber} dateOfBirth={new Date(c.dateOfBirth).toLocaleDateString('en-US')} email={c.email} fullName={c.fullName} skills={c.skills} key={idx} />)}
            </div>
